...
1. **Download or clone** this repository. Run `npm install` and `npm run build` first to generate the `dist/` directory, then load the extension.
...
├── dist/content.js        # Built content script loaded by the manifest (generated by `npm run build`, not committed)
